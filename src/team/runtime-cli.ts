...
import type { StartTeamConfig, TeamRuntime, TeamSnapshot } from './runtime.js';
...
async function main(): Promise<void> {
  ...
  const config: StartTeamConfig = {
    teamName,
    workerCount,
    agentTypes: agentTypes as StartTeamConfig['agentTypes'],
    tasks,
    cwd,
    newWindow,
  };
  ...
  // Start the team — event-driven runtime with CLI API inbox (no done.json, no watchdog)
  try {
    runtime = await startTeam(config);
    workerPaneIds = runtime.config.workers
      .map((w) => w.pane_id)
  ...
  // ── Event-driven poll loop (no watchdog) ────────────────────────────
